...
export default function EditShiftPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (shift.status === "COMPLETED" || shift.status === "CANCELLED") {
        router.replace(`/agency/shifts/${shiftId}`);
        return;
      }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSuccess(true);
    setTimeout(() => router.push(`/agency/shifts/${shiftId}`), 1500);
  }
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <Link href={`/agency/shifts/${shiftId}`} className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-700 mb-6 transition-colors">
